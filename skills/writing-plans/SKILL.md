...
**A step whose code calls a library, external API, or third-party service
carries that call's source inside the block.** Not a style rule:
`scripts/task-brief` extracts the task verbatim and the controller hands it to
the implementer as "the exact values to use verbatim", so a signature you
half-remembered arrives labeled as fact, and the first thing to disagree with
it is the running system.

**Read [dependency-calls.md](references/dependency-calls.md) before writing that
step.** It carries the two citation forms, which symbols need their own source
rather than the spec's, why an unreachable source is not an approval, and the
worked example. A call you could not ground at either source is not a step —
it goes to your human partner.
