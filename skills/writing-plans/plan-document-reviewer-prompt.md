...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Spec:** take the path from the plan's own `**Source spec:**` header
    ...
    | No task delivers a merge, a deploy, or a post-merge smoke run | BLOCKING — those belong to superpowersplus:finishing-a-development-branch, which runs after the audit PASSes. In the plan they deadlock it (no merge option before PASS, no PASS while the task has no evidence) and they refine no spec criterion, which is INVENTED SCOPE at the audit |
