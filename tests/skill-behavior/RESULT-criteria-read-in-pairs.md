...
| **Rule changed since** | 2026-09-04 (88d0864) — measured against earlier text. That change replaces the coverage-matrix row with four rows charging the six Verification Matrix columns, an unresolved instrument, an invented test in a `structural` or `negative` row, and a task block disagreeing with the matrix; it also adds the evidence-model marker rule. **It does not touch the Plan Contract row this record measures** — restricted to that commit's changed lines, its diff of this file carries no `contradict` at all. An unrestricted search of the same diff returns one hit, and it is a context line belonging to a different row, the one beginning "No task contradicts a Global Constraint"; the distinction is the whole reason the restricted form is the one stated here. Earlier: 2026-09-03 (1850fb8), two additions neither touching that row: a **Previous findings** line in the Output Format, and a five-item cap on the Recommendations bucket |
