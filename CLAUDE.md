...
| **`verification-before-completion` sits outside the flow wiring on purpose — one invoker, [`skills/systematic-debugging/SKILL.md`](skills/systematic-debugging/SKILL.md), section "Phase 4: Implementation", and no other — and the reason is a measurement.** [`RESULT-verification-before-completion.md`](tests/skill-behavior/RESULT-verification-before-completion.md) has the verification running in a repository where no skill was reachable. Wiring it further in argues against the record. | The sweep surfaces it beside `dispatching-parallel-agents` and it looks like the one real orphan. Connecting it would ship a rule the measurement says is not what produces the behaviour |
...
- **A `**Rule path**` row in a `RESULT-*` is bare unlinked text, and that is the third form.** It is not an exception to the rule below but a stronger case than either: [`check-skill-behavior-records.sh`](scripts/check-skill-behavior-records.sh) parses the row and checks the file exists, so the path is read by a gate that a markdown link would not have given it. Do not convert these to links — the parser takes the first file-shaped token, and link syntax around it changes what it reads.
