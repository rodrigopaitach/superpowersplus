...
| The Test Coverage Matrix and Task 4's criteria name `nit_cap_present` (`AC5`) and `nit_cap_per_face` (`IR4`) — four places: the two matrix rows and the two acceptance lines | **Both** functions are gone. `capped_faces` carries what each charged for the three capped faces, and a new `uncapped_faces` charges the other half | The rule became two-sided, so one assertion group could not carry it. `check-cross-references` stays green on every one of the four — the old names still occur in this plan's own historical code block — so no gate can see them, and this row is the only marking they get |
