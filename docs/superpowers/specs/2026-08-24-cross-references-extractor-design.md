...
- **IR8** — Every file the branch changes belongs to one of the classes below. **The evidence is the audit's own `git diff --name-status <branch point>..HEAD` read against this list, never a test** — a permanent suite cannot assert a branch's file list: once the branch merges, `merge-base main HEAD` is HEAD and the diff is empty, and on the next branch anyone cuts the assertion charges someone else's files. A gate that can only be vacuous or wrong is not the instrument for this criterion. Reasoned, not measured.
  ...
  **This charging is the audit's work and ships as no script.** A checker pinned to this branch's fork point outlives the branch and then charges the next branch's files against this list; one deriving its base from `merge-base` empties after the merge. Those are the same two failures the paragraph above rules out, and adding a file to the repository does not escape them.
  ...
  - **The `file:line` citations this branch's own edits invalidated, in the live documents of other work** — [`scripts/check-skill-size.sh`](../../../scripts/check-skill-size.sh), [`docs/superpowers/specs/2026-08-21-execution-path-context-budget-design.md`](2026-08-21-execution-path-context-budget-design.md), [`docs/superpowers/plans/2026-08-21-execution-path-context-budget.md`](../plans/2026-08-21-execution-path-context-budget.md), [`docs/superpowers/plans/2026-08-21-upstream-consult-fixes.md`](../plans/2026-08-21-upstream-consult-fixes.md), [`docs/superpowers/specs/2026-08-21-upstream-consult-fixes-design.md`](2026-08-21-upstream-consult-fixes-design.md), [`docs/superpowers/plans/2026-08-24-knowledge-to-skills-traversal.md`](../plans/2026-08-24-knowledge-to-skills-traversal.md) and [`docs/superpowers/specs/2026-08-23-knowledge-to-skills-traversal-design.md`](2026-08-23-knowledge-to-skills-traversal-design.md). **This class named two files until 2026-08-24 and the branch had broken twelve citations in seven.** Two edits caused the cascade: removing six lines from the top of `anthropic-best-practices.md`, and adding comment lines to `check-skill-size.sh`, which pushed `MAX=500` from `:26` to `:34`. The class was never "the two files I had noticed" — it is every citation this branch's own edits invalidated, and it is written that way now. No gate finds these: `check-cross-references` verifies only that a cited file opens and is long enough, and it charges no citation whose path carries no dotted extension.
...
**Renumbering them is what would be wrong, and the distinction is whose document it is.** A design document describes the state it was written against; renumbered to match the code it produced, it reads as if it had described the result, and the record of what was decided before the work disappears. The rule flips for a citation this branch's edit broke in *someone else's* live document, and there it is repaired **exhaustively, not where it was first noticed**: this doctrine was written naming two such files, and a review then found the branch had broken twelve citations across seven. All twelve are repaired, and `check-skill-size.sh` was moved to a section-title anchor besides. The class list below names the seven. **Nothing enforces either side**: `check-cross-references` verifies only that a cited file opens and is long enough, and `check-links.sh` reads no line number and no `.sh` file at all.
