...
- **AC22** — Where a coverage-matrix cell names the suite file as well as the case, the case is charged against that file: a plan whose matrix names a case its own suite does not contain exits 1, even when the plan's own fenced blocks carry the name. A suite path that does not resolve falls back to the code-block search of `AC20` rather than failing, because a plan is written before its tests exist. **Like `AC21`, this criterion was written after the work it describes** — the whole-branch code review measured the gap `AC20` had recorded as an open limit, and the human partner decided on 2026-08-24 to close it rather than carry it. `AC20` is unchanged and still holds; this adds a second, stronger source where the matrix already names one.
- **AC19** — The Codex archive carries the shared module and still contains no `^scripts/` path: [`tests/codex/test-package-codex-plugin.sh`](../../../tests/codex/test-package-codex-plugin.sh) passes on a clean tree, and that suite asserts the module's presence in the extracted archive rather than only the absence of a forbidden prefix.
- **AC21** — A carrier that builds its `--help` from its own header prints **the whole of the part it slices** and stops at the first line of code. For two of the three that is the entire header; [`scripts/sync-to-codex-plugin.sh`](../../../scripts/sync-to-codex-plugin.sh) prints its Usage block to the end of the header by design, and losing the marker that starts that block exits non-zero rather than printing nothing. Neither a line added to the header, nor a blank line inside it, nor a rewording of any sentence in it changes those two properties. **This criterion was written on 2026-08-24, after the work it describes had already been done** — the three carriers were fixed under `IR8`, which permits files to change and says nothing about what they must do, so the work traced to nothing and the conformance audit charged it as scope nobody specified. It is written here rather than waived because a permission is not a requirement: without this line the delivered behaviour has tests holding it and no statement of what it was supposed to be.
...
- **IR4** — Each carrier resolves the module from its own path, not from the working directory. `check-cross-references` run from an unrelated directory against an unrelated repository still works. **The two carriers reach it by different routes, and the criterion covers both:** `check-cross-references` passes its own `$0`-derived directory to python, while [`scripts/check-links.sh`](../../../scripts/check-links.sh) inserts the path relative to the working directory — legitimate because that script has already `cd`-ed to a `REPO_ROOT` derived from its own `$0`, so the cwd IS its own path by then. Written down because the second form reads like the defect the first exists to avoid, and the next reader would otherwise "fix" it.
...
- **IR8** — Every file the branch changes belongs to one of the classes below. **The evidence is the audit's own `git diff --name-status <branch point>..HEAD` read against this list, never a test** — a permanent suite cannot assert a branch's file list: once the branch merges, `merge-base main HEAD` is HEAD and the diff is empty, and on the next branch anyone cuts the assertion charges someone else's files. A gate that can only be vacuous or wrong is not the instrument for this criterion. Reasoned, not measured.
  ...
  **How the audit charges it, and why the class list is written the way it is.** Read the classes below out of this document — each class is one sub-bullet, and its members are the links that name repository paths, wherever in the sub-bullet they sit; a link cited as the SOURCE of a justification is not a member. Read as "only the links before the em dash" — the form first published here — five of the eight classes come out memberless and `IR8` fails on the branch itself; measured on 2026-08-24 over the 25 files the branch changed, 22 then land in no class. Resolve each of those links to a repository path and charge them against `git diff --name-only <branch point>` — the branch point against the WORKING TREE, plus `git ls-files --others --exclude-standard`, never `<branch point>..HEAD`, which sees committed state only and lets any uncommitted violation pass. Measured 2026-08-24: the committed-only form reported PASS on a tree whose uncommitted change touched a file no class named. Charge in three directions: a changed file in no class, a class matching no changed file, and a file matching more than one class. The third is what makes the classes worth naming precisely — it is how a class broader than its own text is caught, since the extra breadth shows up as a file two classes both claim. All three were exercised on 2026-08-24 by deleting a class, adding one the branch never touched, and restoring an earlier directory-prefix form; each produced its own failure, and the branch as it stands produces none.

  **This charging is the audit's work and ships as no script.** A checker pinned to this branch's fork point outlives the branch and then charges the next branch's files against this list; one deriving its base from `merge-base` empties after the merge. Those are the same two failures the paragraph above rules out, and adding a file to the repository does not escape them.
  ...
  - **Their tests** — [`tests/hooks/test-mdfence.sh`](../../../tests/hooks/test-mdfence.sh), [`tests/hooks/test-check-cross-references.sh`](../../../tests/hooks/test-check-cross-references.sh), [`tests/hooks/test-check-links.sh`](../../../tests/hooks/test-check-links.sh), [`tests/hooks/test-task-brief.sh`](../../../tests/hooks/test-task-brief.sh) and [`tests/codex/test-package-codex-plugin.sh`](../../../tests/codex/test-package-codex-plugin.sh), which carries `AC19`'s evidence that the archive holds the module. Named one by one for the reason the spec-and-plan class below gives: `tests/hooks/` permits any file under that tree, and this is the same directory form that silently absorbed another branch's document one class down.
  ...
  - **This branch's own spec and plan** — [`docs/superpowers/specs/2026-08-24-cross-references-extractor-design.md`](2026-08-24-cross-references-extractor-design.md) and [`docs/superpowers/plans/2026-08-24-cross-references-extractor.md`](../plans/2026-08-24-cross-references-extractor.md). The process writes them, and the plan carries its `**Execution:**` field and its progress as the branch runs. **Named one by one and not as `docs/superpowers/`**: the directory form reads as this class and permits any document under that tree, including another branch's spec — measured, it silently absorbed the 2026-08-21 spec that belongs to the citation-repair class below.
  - **The two other carriers that print `--help` by slicing their own header** — [`scripts/lint-shell.sh`](../../../scripts/lint-shell.sh) and [`scripts/sync-to-codex-plugin.sh`](../../../scripts/sync-to-codex-plugin.sh), with their suites — [`tests/shell-lint/test-lint-shell.sh`](../../../tests/shell-lint/test-lint-shell.sh) and [`tests/codex-plugin-sync/test-sync-to-codex-plugin.sh`](../../../tests/codex-plugin-sync/test-sync-to-codex-plugin.sh), named rather than implied. **This class was added on 2026-08-24 by the human partner's explicit instruction, after the branch had already declared both files out of scope and recorded them as leftovers.** It is a widening, not a discovery, and it is written down as one: the first was truncating its `--help` in the released tree, the second was one reworded comment away from dumping its own body, and the fix for both is the shape this branch had just proved in `check-cross-references`. Leaving them to a later branch was the earlier reading; the partner's is that a defect this branch measured, named and knew how to fix does not wait.
  - **The `file:line` citations this branch's own edits invalidated, in the live documents of other work** — [`scripts/check-skill-size.sh`](../../../scripts/check-skill-size.sh), [`docs/superpowers/specs/2026-08-21-execution-path-context-budget-design.md`](2026-08-21-execution-path-context-budget-design.md), [`docs/superpowers/plans/2026-08-21-execution-path-context-budget.md`](../plans/2026-08-21-execution-path-context-budget.md), [`docs/superpowers/plans/2026-08-21-upstream-consult-fixes.md`](../plans/2026-08-21-upstream-consult-fixes.md), [`docs/superpowers/specs/2026-08-21-upstream-consult-fixes-design.md`](2026-08-21-upstream-consult-fixes-design.md), [`docs/superpowers/plans/2026-08-24-knowledge-to-skills-traversal.md`](../plans/2026-08-24-knowledge-to-skills-traversal.md) and [`docs/superpowers/specs/2026-08-23-knowledge-to-skills-traversal-design.md`](2026-08-23-knowledge-to-skills-traversal-design.md). **This class named two files until 2026-08-24; three sweeps were needed to find that the branch had broken thirteen citations in six, and that five more it had also touched were already stale before it.** Two edits caused the cascade: removing six lines from the top of `anthropic-best-practices.md`, and adding comment lines to `check-skill-size.sh`, which pushed `MAX=500` from `:26` to `:34`. The class was never "the two files I had noticed" — it is every citation this branch's own edits invalidated, and it is written that way now. No gate finds these: `check-cross-references` verifies only that a cited file opens and is long enough, and it charges no citation whose path carries no dotted extension.
  - **The two files the whole-branch review's Minor findings named** — [`.gitignore`](../../../.gitignore) and [`docs/testing.md`](../../../docs/testing.md). **This class was added on 2026-08-24 by the human partner's explicit instruction**, after the branch review reported both: the first has no `__pycache__` rule, so the bytecode this branch's module can produce is committable into a `skills/` tree that ships wholesale, and the two `sys.dont_write_bytecode` flags guard two callers by name rather than the class; the second lists seven of the ten gates `.github/workflows/ci.yml` runs, three lines above the paragraph explaining why the suite list was deliberately not written down. The second is not this branch's defect — it predates it — and is repaired here because the partner chose to, which is the same widening the `--help` class above records.
...
**Every `file:line` citation in this document is a PRE-FIX line number**, and they are left that way on purpose: this document was written before the work and records the state the defects were found in. Measured 2026-08-24 against the branch point `0aa28b7`: of its 25 distinct citations (27 as written, 25 after short-form resolution — the count of 23 first published here excluded the two whose path carries no dotted extension, and both are in the nine), **9 now resolve to a different line** — `.github/workflows/ci.yml:156` (the `check-links.sh` step is now at `:162`), `tests/hooks/test-check-links.sh:184` (now `:189`), `skills/writing-plans/scripts/check-cross-references:72`, `skills/subagent-driven-development/scripts/task-brief:28`, `scripts/check-links.sh:74`, two into `anthropic-best-practices.md`, and two into `CHANGELOG.md`, which every release shifts. Reproduce with `git show 0aa28b7:<path>` against `git show HEAD:<path>` for each.

**Renumbering them is what would be wrong, and the distinction is whose document it is.** A design document describes the state it was written against; renumbered to match the code it produced, it reads as if it had described the result, and the record of what was decided before the work disappears. The rule flips for a citation this branch's edit broke in *someone else's* live document, and there it is repaired **exhaustively, not where it was first noticed**: this doctrine was written naming two such files; a review found more, and a third pass found that five of those were never this branch's to repair and had been moved to a wrong line by a wrong offset. Thirteen citations across six documents were broken here and are repaired; the five pre-existing ones were converted to a section-title anchor rather than renumbered — the line moved a third time while the correction was being written, which is what a line citation into a file edited every release does. `check-skill-size.sh` was moved to a section-title anchor besides. **Repairing a citation without opening what it lands on is the same defect as writing it that way.** The class list below names the documents. **Nothing enforces either side**: `check-cross-references` verifies only that a cited file opens and is long enough, and `check-links.sh` reads no line number and no `.sh` file at all.

**The specific shift this branch's `AC16` caused:** this spec records the state the defect was found in. `AC16` removes six lines from the top of that file, so every line below them shifted up by six and `:16`–`:21` no longer exist at all. That shift is the reason [`scripts/check-skill-size.sh`](../../../scripts/check-skill-size.sh) now anchors that file by section title instead of by line — the rule [`CLAUDE.md`](../../../CLAUDE.md) already states for a file this project edits, and one nothing enforces: `check-links.sh` reads no `.sh` file and no line number.
