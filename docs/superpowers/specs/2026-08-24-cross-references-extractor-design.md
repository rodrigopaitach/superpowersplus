...
**Every `file:line` citation in this document is a PRE-FIX line number**, and they are left that way on purpose: this document was written before the work and records the state the defects were found in. Measured 2026-08-24 against the branch point `0aa28b7`: of its 23 distinct citations, **9 now resolve to a different line** — `.github/workflows/ci.yml:156` (the `check-links.sh` step is now at `:162`), `tests/hooks/test-check-links.sh:184` (now `:189`), `skills/writing-plans/scripts/check-cross-references:72`, `skills/subagent-driven-development/scripts/task-brief:28`, `scripts/check-links.sh:74`, two into `anthropic-best-practices.md`, and two into `CHANGELOG.md`, which every release shifts. Reproduce with `git show 0aa28b7:<path>` against `git show HEAD:<path>` for each.

**Renumbering them is what would be wrong, and the distinction is whose document it is.** A design document describes the state it was written against; renumbered to match the code it produced, it reads as if it had described the result, and the record of what was decided before the work disappears. The rule flips for a citation this branch's edit broke in *someone else's* live document — [`scripts/check-skill-size.sh`](../../../scripts/check-skill-size.sh) and the 2026-08-21 spec were repaired for exactly that reason, and `check-skill-size.sh` was moved to a section-title anchor besides. **Nothing enforces either side**: `check-cross-references` verifies only that a cited file opens and is long enough, and `check-links.sh` reads no line number and no `.sh` file at all.
