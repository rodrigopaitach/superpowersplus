...
- **IR2** — A fence opener indented by up to three spaces is recognised, and a tilde fence is recognised on the same terms as a backtick fence.
- **IR3** — Every one of the nine existing cases in [`tests/hooks/test-check-cross-references.sh`](../../../tests/hooks/test-check-cross-references.sh) still passes, unmodified.
- **IR4** — No document under `docs/` changes verdict except as AC4 requires. Measured against the 40 committed specs and plans, before and after.
- **IR5** — Each acceptance criterion's test is shown to fail against the pre-fix script. A test that never went red asserts nothing.
- **IR6** — No file outside the script and its test suite changes behaviour. `CHANGELOG.md` is the only other file edited.
- **IR7** — The script stays zero-dependency: `python3` and `git`, both already required at `:58` and `:218`.
...
- **This repository already counts task headings outside fences, in another script** — [`skills/subagent-driven-development/scripts/task-brief:29`](../../../skills/subagent-driven-development/scripts/task-brief): `/^```/ { infence = !infence }`, and `:30`: `!infence && /^#+[ \t]+Task[ \t]+[0-9]+/ {`. On the same plan it answers 5 where `check-cross-references` answers 17.
...
| Edge cases and failures | Resolved — nested fences (44 four-backtick and 2 five-backtick openers), indented openers (6 files), tilde fences (none present, accepted for one character), unterminated fences (none present today) | IR1, IR2, AC11 |
| Constraints and tradeoffs | Resolved — zero dependencies means a hand-rolled scanner; two naive implementations already exist in this repository and are deliberately not unified with it or corrected here | IR6, IR7, and the first item of `## Assumptions to Confirm` |
...
| Completion signals | Resolved — every criterion is settled by running the script against a named document or a fixture and reading its exit code and summary line, and each test must first be seen red | AC1–AC12, IR3, IR5 |
