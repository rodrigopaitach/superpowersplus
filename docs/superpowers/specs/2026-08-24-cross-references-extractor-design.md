...
- **Whether a fourth carrier needs the scanner was not surveyed beyond fence-tracking code.** Searched: `grep -nE 'fence|infence|in_fence'` over `scripts/`, `skills/*/scripts/`, `githooks/` and `hooks/`, which returns three files — the two carriers named here plus [`scripts/check-no-dispatch.sh`](../../../scripts/check-no-dispatch.sh), whose hits are the word "fenced" inside prose comments rather than fence-tracking logic; both were opened to classify them. A script that reads markdown structurally without any fence handling at all — which is exactly what `check-cross-references` was — would not appear in that search.
