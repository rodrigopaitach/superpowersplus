...
**Measured.** Replacing the mask with the CommonMark rule and running the gate unchanged otherwise moves it from `exit 0 — 515 local link(s) across 104 file(s) resolve` to `exit 1` naming six links: [`skills/writing-skills/anthropic-best-practices.md:16`](../../../skills/writing-skills/anthropic-best-practices.md) through `:21`. Their targets are at `:634`–`:664`, and every one of the six is inside a fenced block — lines of a report template shown as an example. A fenced block produces no heading, so it produces no anchor.
...
- **IR8** — Every file the branch changes is one of: the three carriers, the new module, their tests, `anthropic-best-practices.md`, and `CHANGELOG.md`. No prose of the vendor's document is touched beyond the six table-of-contents lines.
...
- **Whether a fourth carrier needs the scanner was not surveyed beyond fence-tracking code.** Searched: `grep -nE 'fence|infence|in_fence'` over `scripts/`, `skills/*/scripts/`, `githooks/` and `hooks/`, which returns only the two carriers named here. A script that reads markdown structurally without any fence handling at all — which is exactly what `check-cross-references` was — would not appear in that search.
...
| Which route — the short path or the full process? | Full process | Recommended the full process. The five short-path criteria held at the time (one production file, no migration directory, no lockfile, no public contract moved, no money/auth/PII), but they measure blast radius, not what is being edited. The scope has since grown to four carriers and a document, so the criteria no longer hold either. Source: order 1, `skills/writing-plans/plan-document-reviewer-prompt.md:97` |
