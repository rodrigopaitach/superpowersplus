...
| Reclassificar retroativamente os rows históricos de comando? | Não. As matrizes históricas já misturam semânticas que o Evidence Model v2 hoje distingue: rows com test id, rows com comando read-only e rows com `NENHUM`. A classificação exata não é fixada aqui porque nenhum instrumento reproduzível viaja com essa cardinalidade — só a contagem de **seis planos com matriz** foi reproduzida de forma independente. A semântica anterior já reprovava o row de comando, e AC48 preserva o comportamento, não o repara | Preservar literalmente e registrar a descoberta como item aberto | Decisão do parceiro, sobre propriedade qualitativa verificada — a cardinalidade foi retirada na task review por não ter instrumento |
