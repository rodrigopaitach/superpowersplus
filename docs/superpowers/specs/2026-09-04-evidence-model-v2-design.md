...
A regra raiz em [`CLAUDE.md`](../../../CLAUDE.md), seção "How you work here", diz
...
`.claude-plugin/plugin.json`, e roda `scripts/check-links.sh`. O gate mais forte
...
conjunto: afirmou 5 arquivos onde há 23. O comando foi executado; o que falhou
...
- **AC1** `[structural]` — `docs/evidence-model.md` existe e define as três
  delivery classes, a source evidence, a live-document reference, a distinção
  locator/evidence, a distinção current-state/provenance, os dois invariantes da
  cláusula de contenção e o *smallest sufficient range*.
- **AC2** `[structural]` — `docs/evidence-model.md` não contém nenhum bloco
  `## Output Format`.
- **AC3** `[structural]` — `CLAUDE.md` e `AGENTS.md` carregam o princípio de
  evidence-or-zero na forma nova, e os dois textos são idênticos entre si.
...
- **AC5** `[structural]` — `brainstorming/SKILL.md` carrega a regra de adequação
  do instrumento como regra operacional, na seção "Where a Claim Comes From" ou
...
- **AC16** `[structural]` — `final-branch-audit/SKILL.md` usa as colunas
  ...
  duas ocorrências da tabela**, a da seção "The Audit Table" e a de dentro do
  prompt de dispatch.
...
- **AC22** `[structural]` — `task-reviewer-prompt.md` representa verificação de
  critério cujo instrumento não é arquivo de teste.
...
- **AC25** `[structural]` — `README.md` descreve a promessa na forma nova, com a
  liberdade editorial que a divergência deliberada entre showcase e referência já
  permite.
...
- **IR1** `[structural]` — nenhum `SKILL.md` **não isento** excede 500 linhas.
...
- **IR5** `[negative]` — nenhum gate novo passa a exigir fragmento literal em
  citação, e o anchor-fragment gate não é construído.
- **IR6** `[negative]` — nenhum script de gate é criado apenas para tornar
  executável uma regra textual. Gate novo continua exigindo defeito ou risco que
  pague seu custo.
- **IR7** `[structural]` — `scripts/check-docs-sync.sh` continua verde:
...
- **Não há escape de escopo para critério de spec.**
  `skills/final-branch-audit/SKILL.md:78`:
...
- **O auditor já é declarado não-confiante em relatório alheio.**
  `skills/final-branch-audit/SKILL.md:263-264`: *"The plan, the ledger, the
...
- **O contrato atual do audit exclui execução.**
  [`docs/review-scopes.md`](../../review-scopes.md), seção "What each face runs",
...
- **Os dois READMEs de documentação são um par gated; o showcase não.**
  [`docs/docs-and-links.md`](../../docs-and-links.md), seção "Three README-shaped
...
- **A testabilidade do coverage map é definida por `file:line`.**
  ...
  seção "Categories", na row *Completion signals*: *"A criterion no `file:line`
...
| Interaction flow | Resolved | A cadeia spec → plan → audit está na seção The model, com a responsabilidade de cada camada |
...
|---|---|---|---|
| Quem declara a evidence class? | Híbrido restrito: spec declara; auditor aponta inadequação com `EVIDENCE CLASS MISMATCH — BLOCKING` e nunca reclassifica para conceder | A spec declara | Padrão do projeto — `skills/final-branch-audit/SKILL.md:263-264`, *"claims under audit — never evidence"* |
...
| Uma spec com dois planos, ou duas specs? | Duas specs e dois planos | Três fatias, sem enunciar que exigia duas specs | Padrão do projeto — `skills/final-branch-audit/SKILL.md:78`, sem escape de escopo para critério |
| Onde mora a regra de adequação do instrumento? | Dois níveis: princípio no repositório, regra operacional em `brainstorming` e finding no spec reviewer | Só no `CLAUDE.md` do projeto | **Recomendação corrigida pelo parceiro**: o `CLAUDE.md` daqui não alcança quem instala o plugin em outro projeto |
